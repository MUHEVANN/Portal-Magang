    ...
    <form action="{{ url('apply-form') }}" class="bg-red lg:w-3/5" method="post" enctype="multipart/form-data">
        ...
        <div x-show.transition="current_pos == 2">
            ...
            <li x-show="cv_lead.length > 0" class="flex items-center pb-2 pt-2">
                <div :class="{
                    'bg-green-200 text-green-700': processValidateCV(),
                    'bg-red-200 text-red-700': !processValidateCV()
                }"
                    class=" rounded-full p-1 fill-current ">
                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path x-show="processValidateCV()" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M5 13l4 4L19 7" />
                        <path x-show="!processValidateCV()" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <span
                    :class="{
                        'text-green-700': processValidateCV(),
                        'text-red-700': !processValidateCV()
                    }"
                    class="font-medium text-sm ml-3"
                    x-text="processValidateCV() ? 
            ...
            <div x-show="cek_output">
                ...
                <template x-for="(ar, idx) in fields">
                    <div class="border-2 border-slate-100 px-2 py-3 mt-5">
                        ...
                        <input type="file" name="cv" class="input-style" id="cv" x-model="ar.cv" />
                        <p class="text-slate-600">Masukan CV dan Portfolio.</p>
